...
#     (FP32 大算子填满 Vector 数据通路; PWM 用 exp 8192 仅 84%)。
...
SIDECAR="/tmp/dcat-rNPU_aiv_load-$chip.pid"
...
case "${DCAT_OP:-inject}" in
    inject)
        ...
        rm -f "$LOG"
        echo "AIVector stress started on chip $chip (dev $dev_id, pid $pid, load=${load_pct}%)"
        ;;
    clean)
